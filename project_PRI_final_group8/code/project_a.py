...
def main():
    ...
    #data.to_csv('processed_data.csv', sep='\t', encoding='utf-8') to store data for inspection to analyze results
    ...
    for arg in sys.argv:
        if arg == 'BM25':
            ...
            #to test if it's better to consider diferent B values for each field to be searched or not
            print("Using BM25F as scoring criteria with common default values B=0.75, k1=1.5 and content_B = [0.75, 0, 0.75, 0, 1] for the fields")
            w = scoring.BM25F(B=0.75, K1=1.5, content_B = [0.75, 0, 0.75, 0, 1])
            searchQuery(query, index, w, number_docs)
        elif arg == 'TF-IDF':
            ...
            #used only for inspection
            print("Using Frequency as scoring criteria")
            w = scoring.Frequency()
            searchQuery(query, index, w, number_docs)
        elif arg == 'Cosine':
            print("Using Cosine similarity as scoring criteria")
            w = scoring.Cosine()
...
#to groups all documents (line in csv) in respective manifesto
...
def createIndex(data):
    ...
    schema = Schema(text=TEXT(analyzer=analyzer, vector=vector_format), manifesto_id=ID(stored=True, sortable=True), party=TEXT(stored=True, analyzer=analyzer, vector=vector_format, sortable=True), date=NUMERIC(stored=True), title=TEXT(stored=True, analyzer=analyzer, vector=vector_format))
...
def searchQuery(arg, index, w, n):
    ...
    with ix.searcher(weighting = w) as searcher:
        ...
        query_parser.add_plugin(MultifieldPlugin(["title","text", "party"]))
        # OrGroup -> so that any of the terms may be present for a document to match
        # AndGroup -> so that all terms need to be present for a document to match
        ...
        docs = []
        print("Manifesto and respective score")
        for result in results:
            print("%s - %s" % (result['manifesto_id'],result.score))
...
def keyword_times_party(index, arg):
    ...
    with index.searcher(weighting = scoring.Frequency()) as searcher:
        ...
        #not included the party as we wish the mentions made by a party, in the text and title of the manifestos
...
def year_relevant_keyword(index, arg):
    ...
    with index.searcher(weighting = scoring.Frequency()) as searcher:
        ...
        query_parser.add_plugin(MultifieldPlugin(["title","text"]))
        #not included the party as we wish to know the year were a certain keyword was mentioned the most, in the text and title of the manifestos
        number_times = {}
        ...
        for result in results:
            ...
            if year not in number_times:
                ...
                number_times[year] += result.score
        get_year(number_times, arg)

#determine year in dictionary that corresponds to the maximum
def get_year(number_times, arg):
    year_max = max(number_times.items(), key=operator.itemgetter(1))[0]
    print("Year and respective number of times for theme")
