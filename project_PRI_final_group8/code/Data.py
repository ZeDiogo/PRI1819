...
class Data:
    ...
    def createVectors(self, stopwords=False, useidf=False, ngram=(1,1), preprocessor=None):
        if (stopwords==True):
            self.vectorizer = TfidfVectorizer(use_idf=useidf, stop_words='english', ngram_range=ngram, min_df=3, max_df=0.9, preprocessor=preprocessor)
        else:   
            self.vectorizer = TfidfVectorizer(use_idf=useidf, min_df=3, max_df=0.9, ngram_range=ngram, preprocessor=preprocessor)
